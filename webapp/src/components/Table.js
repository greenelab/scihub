...
export const NumberCell = ({ value, decimals = 0 }) => (
  <span>{format.number(value, decimals)}</span>
);

export const PercentCell = ({ value }) => (
  <span className={styles.percent}>{format.percent(value)}</span>
);
...
export const TableHeaderTip = ({ tooltip }) => (
...
export const TooltipHeading = ({ title, tooltip, icon }) => {
  return (
    <a className={styles.header} href="javascript:void(0)">
      {title}
...
export const CreateTooltipHeader = tooltip => props => (
  <TooltipHeading {...props} tooltip={tooltip} />
);
