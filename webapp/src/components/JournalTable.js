...
export default class JournalTable extends FetchDataTable {
  static Tooltips = {
    ...
    scihub:
      "The total number of the journal's articles which are in Sci-Hub's database.",
    crossref:
      'The total number of articles published by the journal, as extracted from Crossref.',
    coverage:
      'The number of articles in Sci-Hub divided by the total number of articles.'
  };

  sortProperties() {
    return [{ id: 'crossref', sortAscending: false }];
  }
    ...
    return (
      <div>
        ...
        <p className="section-description">
          The following table shows Sci-Hub's article coverage, as of March
          2017, for each journal.
        </p>
    ...
    return (
      <RowDefinition>
        <ColumnDefinition
          ...
          title=" "
          width="20px"
          customComponent={ActiveJournalCell}
        />
        <ColumnDefinition
          id="title_name"
          title="Journal"
          width="70%"
          customComponent={JournalCell}
          customHeadingComponent={CreateTooltipHeader(
            JournalTable.Tooltips.title_name
          )}
        />
        <ColumnDefinition
          id="open_access"
          title=" "
          width="20px"
          customComponent={OpenAccessJournalCell}
        />
        <ColumnDefinition
          id="scihub"
          title="Sci-Hub"
        ...
        <ColumnDefinition
          ...
          customComponent={NumberCell}
          customHeadingComponent={CreateTooltipHeader(
            JournalTable.Tooltips.crossref
          )}
        />
        <ColumnDefinition
          id="coverage"
          ...
          customHeadingComponent={CreateTooltipHeader(
            JournalTable.Tooltips.coverage
          )}
        />
      </RowDefinition>
    );
...
let JournalCell = ({ value, rowData }) => (
  <div>
    <Link to={`/journal/${rowData.scopus_id}`} className="btn-link">
      {value}
    </Link>
  </div>
);
...
export let OpenAccessJournalCell = ({ value, className }) => (
  <span className={className}>
    {value && (
      <Tooltip title="The articles of this journal are free to read.">
        <img
          src="https://upload.wikimedia.org/wikipedia/commons/7/77/Open_Access_logo_PLoS_transparent.svg"
          className="open-access-logo"
          alt="open access journal"
        />
      </Tooltip>
    )}
  </span>
